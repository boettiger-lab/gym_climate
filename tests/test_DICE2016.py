...
def fOBJ(x,sign,iI,iK,ial,il,iYGROSS,isigma,iEIND,iE,iCCA,iCCATOT,icumetree,iMAT,iMU,iML,iFORC,iTATM,iTOCEAN,iDAMFRAC,iDAMAGES,iABATECOST,icost1,iMCABATE,
    ...
    with open('parameter_check.csv', 'w') as myfile:
        for parameter in [iK, iYGROSS, iEIND, iE, iCCA,\
...
if __name__ == '__main__':
    ...
    bnds = bnds1 + bnds2
    x = np.ones(2*NT)/4
    print(fOBJ(x, 1.0,I,K,al,l,YGROSS,sigma,EIND,E,CCA,CCATOT,cumetree,MAT,MU,ML,FORC,TATM,TOCEAN,DAMFRAC,DAMAGES,ABATECOST,cost1,MCABATE,CPRICE,YNET,Y,C,CPC,PERIODU,CEMUTOTPER,RI,NT))
