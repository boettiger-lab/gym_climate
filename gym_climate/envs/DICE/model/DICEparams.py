...
class DICEparams:
    def __init__(self, t, duration=100):
        ...
        self.pop0 = (
            7403  # Initial world population 2015 (millions)          /7403 /
        )
        self.popadj = (
            0.134  #  Growth rate to calibrate to 2050 pop projection  /0.134/
        )
        self.popasym = (
            11500  # Asymptotic population (millions)                 /11500/
        )
        ...
        self.dela = (
            0.005  #   Decline rate of TFP per 5 years                  /0.005/
        )
        ...
        self.mateq = (
            588  # mateq Equilibrium concentration atmosphere  (GtC)    /588  /
        )
        self.mueq = (
            360  # mueq Equilibrium concentration in upper strata (GtC) /360  /
        )
        ...
        self.sig0 = (
            None  # Carbon intensity 2010 (kgCO2 per output 2005 USD 2010)
        )

        # ** Climate model parameters
        self.t2xco2 = (
            3.1  # Equilibrium temp impact (oC per doubling CO2)    / 3.1 /
        )
        self.fex0 = (
            0.5  #   2015 forcings of non-CO2 GHG (Wm-2)              / 0.5 /
        )
        self.fex1 = (
            1.0  #   2100 forcings of non-CO2 GHG (Wm-2)              / 1.0 /
        )
        self.tocean0 = (
            0.0068  # Initial lower stratum temp change (C from 1900) /.0068/
        )
        self.tatm0 = (
            0.85  #  Initial atmospheric temp change (C from 1900)    /0.85/
        )
        self.c1 = 0.1005  #     Climate equation coefficient for upper level  /0.1005/
        self.c3 = (
            0.088  #     Transfer coefficient upper to lower stratum    /0.088/
        )
        self.c4 = (
            0.025  #     Transfer coefficient for lower level           /0.025/
        )
        self.fco22x = 3.6813  # eta in the model; Eq.22 : Forcings of equilibrium CO2 doubling (Wm-2)   /3.6813 /

        # ** Climate damage parameters
        self.a10 = (
            0  #     Initial damage intercept                         /0   /
        )
        self.a20 = None  #     Initial damage quadratic term
        self.a1 = (
            0  #      Damage intercept                                 /0   /
        )
        self.a2 = (
            0.00236  #      Damage quadratic term                     /0.00236/
        )
        self.a3 = (
            2.00  #      Damage exponent                              /2.00   /
        )

        # ** Abatement cost
        self.expcost2 = 2.6  # Theta2 Exponent of control cost function             / 2.6  /
        self.pback = (
            550  #   Cost of backstop 2010$ per tCO2 2015          / 550  /
        )
        self.gback = (
            0.025  #   Initial cost decline backstop cost per period / .025/
        )
        self.limmiu = (
            1.2  #  Upper limit on control rate after 2150        / 1.2 /
        )
        self.tnopol = (
            45  #  Period before which no emissions controls base  / 45   /
        )
        self.cprice0 = (
            2  # Initial base carbon price (2010$ per tCO2)      / 2    /
        )
        self.gcprice = (
            0.02  # Growth rate of base carbon price per year     /.02  /
        )
        ...
        self.ga = self.ga0 * np.exp(
            -self.dela * 5 * (self.times)
        )  # TFP growth rate dynamics
        self.pbacktime = self.pback * (1 - self.gback) ** (
            self.times
        )  # Backstop price
        self.etree = self.eland0 * (1 - self.deland) ** (
            self.times
        ...
        if self.t > 17:
            self.forcoth[self.t] = self.forcoth[self.t] + (
                self.fex1 - self.fex0
            )
        else:
            self.forcoth[self.t] = self.forcoth[self.t] + (1 / 17) * (
                self.fex1 - self.fex0
            ) * (self.t)
        self.optlrsav = (
            (self.dk + 0.004)
            / (self.dk + 0.004 * self.elasmu + self.prstp)
            * self.gama
        )  # Optimal long-run savings rate used for transversality (Question)
        self.cpricebase = self.cprice0 * (1 + self.gcprice) ** (
            5 * (self.times)
        )
