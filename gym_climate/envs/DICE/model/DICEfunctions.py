...
class DICEfunctions(DICEparams):
    ...
    def fFORC(self, iMAT, index):
        return (
            self.fco22x * np.log(iMAT[index] / 588.000) / np.log(2)
            + self.forcoth[index]
        )
    ...
    def fMAT(self, iMAT, iMU, iE, index):
        if index == 0:
            ...
            return (
                iMAT[index - 1] * self.b11
                + iMU[index - 1] * self.b21
                + iE[index - 1] * 5 / 3.666
            )
    ...
    def fMU(self, iMAT, iMU, iML, index):
        if index == 0:
            ...
            return (
                iMAT[index - 1] * self.b12
                + iMU[index - 1] * self.b22
                + iML[index - 1] * self.b32
            )
    ...
    def fTATM(self, iTATM, iFORC, iTOCEAN, index):
        if index == 0:
            ...
            return iTATM[index - 1] + self.c1 * (
                iFORC[index]
                - (self.fco22x / self.t2xco2) * iTATM[index - 1]
                - self.c3 * (iTATM[index - 1] - iTOCEAN[index - 1])
            )
    ...
    def fTOCEAN(self, iTATM, iTOCEAN, index):
        if index == 0:
            ...
            return iTOCEAN[index - 1] + self.c4 * (
    ...
    def fK(self, iK, iI, index):
        if index == 0:
            ...
            return (1 - self.dk) ** self.tstep * iK[
                index - 1
            ] + self.tstep * iI[index - 1]

    # Interest rate equation; Eq. 26 added in personal notes
    def fRI(self, iCPC, index):
        return (1 + self.prstp) * (iCPC[index + 1] / iCPC[index]) ** (
            self.elasmu / self.tstep
        ) - 1
    ...
    def fPERIODU(self, iC, il, index):
        return ((iC[index] * 1000 / il[index]) ** (1 - self.elasmu) - 1) / (
            1 - self.elasmu
        ) - 1
    ...
    def fUTILITY(self, iCEMUTOTPER, resUtility):
        resUtility[0] = (
            self.tstep * self.scale1 * np.sum(iCEMUTOTPER) + self.scale2
        )
